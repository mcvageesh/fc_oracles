...
predictions_save_path = 'results/predictions_rmse_non_global_' + str(random_seed) + '_' + str(use_random_training) + '.csv'
...
if run_hp_search:
    for lge in test_leagues:
        hp_search_save_path = 'results/hp_search_rmse_non_global_' + lge + '_' + str(random_seed) + '_' + str(use_random_training) + '.pickle'
        space = {
    ...
    for lge in test_leagues:
        ...
        for k in range(num_random_seeds):
            for i in range(5):
                ...
                pred_y += tmp.predict(val_x)
        pred_y = pred_y / (num_random_seeds * 5)
        test_data_df_copy.loc[test_data_df_copy['Lge'] == lge, ['prd_HS', 'prd_AS']] = np.round(10 * pred_y)
