...
hp_search_save_path = 'results/hp_search_rps_global_' + str(random_seed) + '_' + str(use_random_training) + '_' \
                      + str(reproduce_submission) + '.pickle'
predictions_save_path = 'results/predictions_rps_global_' + str(random_seed) + '_' + str(use_random_training) + '_' \
    ...
    def objective(space):
        ...
        for k in range(num_random_seeds):
            ...
            if reproduce_submission:
                ...
                model = MyModel(space, TRAIN_X.shape[1], embedding_input_dim=len(test_leagues))
...
if eval_on_test:
    ...
    for k in range(num_random_seeds):
        ...
        if reproduce_submission:
            ...
            model = MyModel(best_hp, TRAIN_X.shape[1], embedding_input_dim=len(test_leagues))
