...
import pandas as pd
import numpy as np
from utils import avg_rps, ranked_probability_loss, parse_boolean
...
predictions_save_path = 'results/predictions_rps_non_global_' + str(random_seed) + '_' + str(use_random_training) + '.csv'
...
if run_hp_search:
    for lge in test_leagues:
        hp_search_save_path = 'results/hp_search_rps_non_global_' + lge + '_' + str(random_seed) + '_' + str(use_random_training) + '.pickle'
        space = {
        ...
        def objective(space):
            ...
            train_df[['gd_ht', 'gd_at', 'gd_ht2', 'gd_at2', 'gd_ht3', 'gd_at3']] = result

            train_df.dropna(inplace=True)

            x = train_df.loc[:, ['gd_ht', 'gd_at', 'gd_ht2', 'gd_at2', 'gd_ht3', 'gd_at3']].values
            y = train_df.loc[:, ['outcome_W', 'outcome_D', 'outcome_L']].values.astype('float32')
            ...
            for k in range(num_random_seeds):
                ...
                model = MyModel(space, train_x.shape[1], embedding_input_dim=None)
            ...
            val_score = avg_rps(val_y, pred_y)
...
if eval_on_test:
    for lge in test_leagues:
        ...
        val_df[['gd_ht', 'gd_at', 'gd_ht2', 'gd_at2', 'gd_ht3', 'gd_at3']] = result
        val_df.dropna(inplace=True)

        val_x = val_df.loc[:, ['gd_ht', 'gd_at', 'gd_ht2', 'gd_at2', 'gd_ht3', 'gd_at3']].values
        pred_y = np.zeros(shape=(val_x.shape[0], 3))
        for k in range(num_random_seeds):
            for i in range(5):
